...
The following API route handles requests to generate responses using HITL. It uses the `streamText` function with tools for scheduling meetings and processes user confirmations.
...
The `ChatHitl` component demonstrates how to integrate HITL into a Next.js application. It uses the `useChat` hook from the Vercel AI SDK to handle API requests and display the AI-generated responses.
...
- [Human in the Loop Documentation](https://sdk.vercel.ai/docs/hitl)